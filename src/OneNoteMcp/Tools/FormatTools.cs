...
public static class FormatTools
{
    ...
    {
        var path = ResolveToFilePath(pathOrNodeId);
        if (path is null)
        ...
        [Description("Target .one file path for the republished current-format section.")] string outputPath)
    {
        try
        {
            ...
            var exported = ExportTools.ExportOne(sectionId, outputPath);
    ...
    /// via the hierarchy. Returns null when nothing resolves to an existing file.
    /// </summary>
    private static string? ResolveToFilePath(string pathOrNodeId)
    {
        ...
        try
        {
            var xml = OneNoteSession.Instance.GetHierarchy(pathOrNodeId, OneNoteScope.HsSelf, OneNoteXmlSchema.Xs2013);
