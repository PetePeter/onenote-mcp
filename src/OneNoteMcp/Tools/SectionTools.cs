...
public static class SectionTools
{
    ...
    public static string CreateSection(
        ...
        ToolVersion.Guarded(version, Capability.OpenHierarchy, s =>
        {
            var sectionPath = name.EndsWith(".one", StringComparison.OrdinalIgnoreCase)
                ? name
                : name + ".one";
            return s.OpenHierarchy(sectionPath, parentId, OneNoteCreateFileType.CftSection);
        });
    ...
    public static string RenameNode(
        ...
        ToolVersion.Guarded(version, Capability.UpdateHierarchy, s =>
        {
            ...
            // rebuilding it.
            var xml = s.GetHierarchy(id, OneNoteScope.HsSelf, OneNoteXmlSchema.Xs2013);
            var root = XDocument.Parse(xml).Root!;
            root.SetAttributeValue("name", newName);
            root.RemoveNodes(); // drop children; keep this node's attributes only
            s.UpdateHierarchy(root.ToString(SaveOptions.DisableFormatting), OneNoteXmlSchema.Xs2013);
            return "{\"renamed\":true}";
        });
